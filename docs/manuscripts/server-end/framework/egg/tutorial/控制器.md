...
permalink: /manuscripts/server-end/framework/egg-tutorial/controller.html
---

# 控制器

在传统的`MVC`架构中，C可以理解为控制器`Controller`，而在Egg框架中服务是在控制器的下发中分层抽离出来的，因此这里一起总结一下。

## 控制器

Egg框架的router将用户的请求基于 `method` 和 `URL` 分发到了对应的 `Controller` 上，`Controller`负责解析用户的输入，处理后返回相应的结果。
