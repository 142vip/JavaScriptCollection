...
单一职责原则要求一个类或模块应该有且仅有一个引起它变化的原因。**一个类应该只负责一项特定的职责**。这样做可以提高代码的可理解性、可维护性和可测试性。
...
`UserProcessor` 类负责处理用户，它首先通过 `UserService` 获取用户信息，然后执行用户处理逻辑，并使用 `EmailService` 发送欢迎邮件给用户
